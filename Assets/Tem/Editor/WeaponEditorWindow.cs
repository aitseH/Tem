...
namespace Tem {
    ...
    public class WeaponEditorWindow : TemEditorWindow {
        ...
        Vector2 DrawWeaponConfigurator(float startX, float startY, Weapon weapon) {
            ...
            TemEditorUtility.DrawSprite(new Rect(startX, startY, 60, 60), weapon.icon);
            startX += 65;

            float offsetY = TemEditor.IsPrefab(weapon.gameObject) ? 5 : 0;
...
//
//            TemEditorUtility.DrawSprite(new Rect(startX+spaceX+width-40, startY+spaceY-45, 40, 40), abilityIdx>0 ? abilityDB.abilityList[abilityIdx-1].icon : null);
...
//            //else weapon.ability=null;
        ...
        int _NewItem(Weapon weapon) {
            if (weaponDB.weaponList.Contains(weapon))
                return selectID;
        ...
        void SelectItem() {
            if (weaponDB.weaponList.Count <= 0)
                return;
